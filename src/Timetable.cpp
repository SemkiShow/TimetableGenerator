...
void SaveTimetable(std::string path, Timetable* timetable)
{
    ...
    for (auto& lesson: timetable->lessons)
    {
        ...
        {
            jsonObject.objectPairs["lessons"].objectPairs[std::to_string(lesson.first)].objectPairs["classIDs"].ints.push_back(lesson.second.classIDs[i]);
        }
        ...
        {
            jsonObject.objectPairs["lessons"].objectPairs[std::to_string(lesson.first)].objectPairs["classroomIDs"].ints.push_back(lesson.second.classroomIDs[i]);
        }
    }
    ...
    for (auto& teacher: timetable->teachers)
    {
        ...
        for (int i = 0; i < daysPerWeek; i++)
        {
            ...
            jsonObject.objectPairs["teachers"].objectPairs[std::to_string(teacher.first)].objectPairs["workDays"].objects[i].format = JSON_INLINE;
            // Lesson number
            for (int j = 0; j < teacher.second.workDays[i].lessonIDs.size(); j++)
            {
                jsonObject.objectPairs["teachers"].objectPairs[std::to_string(teacher.first)].objectPairs["workDays"].objects[i].ints.push_back(
    ...
    for (auto& classPair: timetable->classes)
    {
        ...
        for (auto& timetableLesson: classPair.second.timetableLessons)
        {
            ...
            for (int j = 0; j < timetableLesson.second.lessonTeacherPairs.size(); j++)
            {
                ...
                jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["lessons"].objectPairs[std::to_string(timetableLesson.first)].objects[j].type = JSON_OBJECT;
                jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["lessons"].objectPairs[std::to_string(timetableLesson.first)].objects[j].format = JSON_INLINE;
                jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["lessons"].objectPairs[std::to_string(timetableLesson.first)].objects[j].intPairs["lessonID"] =
                    timetableLesson.second.lessonTeacherPairs[j].lessonID;
                jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["lessons"].objectPairs[std::to_string(timetableLesson.first)].objects[j].intPairs["teacherID"] =
        ...
        for (int i = 0; i < daysPerWeek; i++)
        {
            ...
            jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["lessonNumbers"].objects[i].format = JSON_INLINE;
            for (int j = 0; j < classPair.second.days[i].lessons.size(); j++)
            {
                jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["lessonNumbers"].objects[i].bools.push_back(
                    classPair.second.days[i].lessons[j]);
            }

            // Days
            jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["days"].objects.push_back(JSONObject());
            jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["days"].objects[i].type = JSON_LIST;
            for (int j = 0; j < classPair.second.days[i].classroomLessonPairs.size(); j++)
            {
                jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["days"].objects[i].objects.push_back(JSONObject());
                jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["days"].objects[i].objects[j].type = JSON_OBJECT;
                jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["days"].objects[i].objects[j].format = JSON_INLINE;
                jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["days"].objects[i].objects[j].intPairs["timetableLessonID"] =
                    classPair.second.days[i].classroomLessonPairs[j].timetableLessonID;
                jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["days"].objects[i].objects[j].objectPairs["classroomIDs"] = JSONObject();
                jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["days"].objects[i].objects[j].objectPairs["classroomIDs"].type = JSON_LIST;
                jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["days"].objects[i].objects[j].objectPairs["classroomIDs"].format = JSON_INLINE;
                for (int k = 0; k < classPair.second.days[i].classroomLessonPairs[j].classroomIDs.size(); k++)
                {
                    jsonObject.objectPairs["classes"].objectPairs[std::to_string(classPair.first)].objectPairs["days"].objects[i].objects[j].objectPairs["classroomIDs"].ints.push_back(
    ...
    *timetable = Timetable();
    ...
    for (auto& classPair: jsonObject.objectPairs["classes"].objectPairs)
    {
        ...
        for (int i = 0; i < classPair.second.objectPairs["days"].objects.size(); i++)
        {
            ...
            {
                timetable->classes[classID].days[i].classroomLessonPairs.push_back(ClassroomLessonPair());
                timetable->classes[classID].days[i].classroomLessonPairs[j].timetableLessonID =
                    classPair.second.objectPairs["days"].objects[i].objects[j].intPairs["timetableLessonID"];
                for (int k = 0; k < classPair.second.objectPairs["days"].objects[i].objects[j].objectPairs["classroomIDs"].ints.size(); k++)
                {
                    timetable->classes[classID].days[i].classroomLessonPairs[j].classroomIDs.push_back(
                        classPair.second.objectPairs["days"].objects[i].objects[j].objectPairs["classroomIDs"].ints[k]);
                }
            }
        }
    }
    ...
    for (int i = 1; i < 3; i++)
    {
        for (int j = 0; j < classLettersPerClassNumber; j++)
        {
            ...
            for (int k = 0; k < 5; k++)
            {
                ...
                timetable->classes[classID].timetableLessons[k].lessonTeacherPairs.push_back(LessonTeacherPair());
                {
                    auto it = timetable->lessons.begin();
                    std::advance(it, rand() % timetable->lessons.size());
                    timetable->classes[classID].timetableLessons[k].lessonTeacherPairs[0].lessonID = it->first;
                }
                {
                    auto it = timetable->teachers.begin();
                    std::advance(it, rand() % timetable->teachers.size());
            ...
            for (int k = 0; k < daysPerWeek; k++)
            {
                ...
                for (int m = 0; m < lessonsPerDay; m++)
                {
                    ...
                    timetable->classes[classID].days[k].classroomLessonPairs[m].timetableLessonID = it->first;
                    for (int n = 0; n < it->second.lessonTeacherPairs.size(); n++)
                    {
                        ...
                        timetable->classes[classID].days[k].classroomLessonPairs[m].classroomIDs.push_back(classroomID);
                    }
                }
            }
        }
